...
globalThis.HabiteseApp.NotificationInspector = {
    ...
    parseNotificationMessage(message = "") {
        ...
        const cci = (texto.match(/CCI:\s*([A-Za-z0-9]+)/i) || [])[1] || "";
        const proprietario = (texto.match(
            /cujo\s+(?:propriet(?:a|á)rio\(a\)|proprietario\(a\))\s+principal\s+(?:é|e):\s*(.*?)\s+teve\s+o\s+arquivo:/i
        ) || [])[1] || "";
        const arquivo = (texto.match(
            /teve\s+o\s+arquivo:\s*(.*?)\s+(?:Alterado\/Cadastrado|Alterado|Cadastrado)\s+na\s+data\s+de/i
        ) || [])[1] || "";
        return { cci: cci.trim(), proprietario: proprietario.trim(), arquivo: arquivo.trim() };
    },
    ...
    extrairNotificacoes() {
        ...
        linhas.forEach(linha => {
            ...
            const textoLabel = label ? label.innerText.replace(/\s+/g, " ").trim() : "";

            if (!textoLabel && !campo) return;

            if (textoLabel === "Título:" && campo) {
                if (atual && atual.titulo) notificacoes.push(atual);
                atual = {
                    id: `notificacao-${notificacoes.length + 1}`,
                    titulo: campo.value.trim(),
                    mensagem: "", data: "", link: "",
                    lida: false, tipo: "informacao", origem: "BSIT",
                    cci: "", proprietario: "", arquivo: ""
                };
                return;
            }
...
globalThis.HabiteseApp.NotificationDomService = {
    ...
    popupEstaVisivel() {
        try {
            ...
            if (!popup && !popupContainer) return false;
            const displayPopup = popup ? window.getComputedStyle(popup).display : "none";
            const displayContainer = popupContainer ? window.getComputedStyle(popupContainer).display : "none";
            return displayPopup !== "none" || displayContainer !== "none";
        } catch (erro) {
    ...
    async abrirPopup() {
        ...
        if (popupJaAberta) return { aberta: true };
        ...
        try {
            ...
            return { erro: "Nao foi possivel abrir o popup de notificacoes." };
        }
    ...
    aguardarPopup() {
        return new Promise((resolve, reject) => {
            ...
            if (jaVisivel) { resolve({ popup, popupContainer }); return; }
    ...
    extrairNotificacoes() {
        return globalThis.HabiteseApp.NotificationInspector.extrairNotificacoes();
    },

    async sync() {
        const resultado = await this.obterNotificacoes();
        return { ...resultado, sincronizadoEm: new Date().toISOString() };
    },
    ...
    async obterNotificacoes() {
        ...
        try {
            ...
            if (!popupJaAberta) {
                ...
                if (abertura && abertura.erro) return abertura;
            }
